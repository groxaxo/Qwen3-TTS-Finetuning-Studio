...
# Prefer the official import surface when available.
try:
    ...
    for epoch in range(args.num_epochs):
        for step, batch in enumerate(dl):
            with accelerator.accumulate(model):
                ...
                hidden_states = get_last_hidden(outputs)

                # Align mask + targets (shift BOTH)
                mask = codec_mask[:, 1:]
                talker_hidden_states = hidden_states[mask]
                talker_codec_ids = codec_ids[:, 1:, :][mask]
